#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
#  run-harness-full.sh — Multi-Epoch Evolution Harness for Codex CLI
#
#  Architecture:
...
# ── Resolve WORK_ROOT (Codex -C + cwd): demo tree or repo/.harness ──
RESUME=false
USER_GOAL=""
ADD_GOAL=""
HARNESS_PROJECT_ARG=""
...
# ── Load Codex runner library ──────────────────────────────────────
source "${SCRIPT_DIR}/lib/codex-runner.sh"
...
while (( CURRENT_EPOCH <= MAX_EPOCHS )); do
  ...
  if [[ ! -f artifacts/spec.md ]]; then
    ...
    run_codex_no_browser "Planner" "$prompt" "artifacts/spec.md"
  ...
  if ! $SKIP_BUILD; then
    ...
    for (( sprint=START_FROM_SPRINT; sprint<=TOTAL_SPRINTS; sprint++ )); do
      ...
      if [[ ! -f "$CONTRACT_FILE" ]]; then
        ...
        run_codex_no_browser "Contract Sprint ${sprint}" "$prompt" "$CONTRACT_FILE"

        if [[ ! -f "$CONTRACT_FILE" ]]; then
          echo "  WARNING: Contract file not created. Retrying once..."
          run_codex_no_browser "Contract Sprint ${sprint} (retry)" "$prompt" "$CONTRACT_FILE"
        fi
        if [[ ! -f "$CONTRACT_FILE" ]]; then
      ...
      if [[ ! -f "$HANDOFF_FILE" ]] && [[ ! -f "$LAST_QA_FOR_SPRINT" ]]; then
        ...
        run_codex_no_browser "Generator Sprint ${sprint}" "$prompt"
      else
      ...
      for (( qa_round=1; qa_round<=EFFECTIVE_MAX_QA; qa_round++ )); do
        ...
        run_codex_with_browser "Evaluator Sprint ${sprint} R${qa_round}" "$prompt" "$QA_REPORT"
        ...
        if [[ "$verdict" == "true" ]]; then
          ...
          if (( qa_round < EFFECTIVE_MAX_QA )); then
            ...
            run_codex_no_browser "Generator Fix Sprint ${sprint} R${qa_round}" "$prompt"
  ...
  if [[ ! -f "$REVIEW_FILE" ]]; then
    ...
    run_codex_with_browser "Product Reviewer Epoch ${CURRENT_EPOCH}" "$prompt" "$REVIEW_FILE"

    if [[ ! -f "$REVIEW_FILE" ]]; then
      echo "  WARNING: Product review not written after ${CODEX_MAX_RETRIES} attempts."
      if [[ -n "${LEGACY_PROJECT:-}" ]]; then
  ...
  if [[ "$MEETS" != "true" && -f "$REVIEW_FILE" ]]; then
    ...
    while (( POLISH_ROUND < MAX_POLISH_ROUNDS )); do
      ...
      if [[ ! -f "$POLISH_CONTRACT" ]]; then
        ...
        run_codex_no_browser "Polish Contract ${POLISH_ROUND}" "$prompt" "$POLISH_CONTRACT"
      ...
      if [[ ! -f "$POLISH_HANDOFF" ]]; then
        ...
        run_codex_no_browser "Polish Generator ${POLISH_ROUND}" "$prompt"
      fi
      ...
      if [[ ! -f "$FAST_VERIFY" ]]; then
        ...
        run_codex_with_browser "Polish Fast Verify ${POLISH_ROUND}" "$prompt" "$FAST_VERIFY"
      fi
      ...
      if [[ ! -f "$POLISH_REVIEW" ]]; then
        ...
        run_codex_with_browser "Re-review after Polish ${POLISH_ROUND}" "$prompt" "$POLISH_REVIEW"
...
IMPORTANT: Actually modify artifacts/spec.md. Do NOT just describe the changes."
    ...
    run_codex_no_browser "Evolution Planner: ${NEXT_GOAL}" "$EVOLUTION_PROMPT"
...
# ═══════════════════════════════════════════════════════════════════
#  Codex CLI Harness — end of script
# ═══════════════════════════════════════════════════════════════════"
echo "═══════════════════════════════════════════════════════════"
