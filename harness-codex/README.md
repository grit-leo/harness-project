...
编排脚本与 Prompt 模板位于本目录；运行 Codex 时**工作目录**为「工作区」`WORK_ROOT`（本目录或历史仓库下的 `.harness`）。

## 脚本入口

- **`run-harness-full.sh`**（推荐）：多 Epoch 编排（构建 → 产品评审 → Polish → 演进队列），支持 `--project` / `--resume` / `--add-goal`。
- **`run-harness-codex.sh`**：较早的单线流程（Planner → Contract → Generator…），仅在本目录树内使用；新需求优先用 `run-harness-full.sh`。
...
| 变量含义 | 路径 | 用途 |
|----------|------|------|
| `SCRIPT_DIR` | 本脚本所在目录（即 `harness-codex/`） | 读取 [`prompts/templates/`](prompts/templates)、[`lib/`](lib)；**始终不变**，与外仓无关。 |
| `WORK_ROOT` / `ROOT` | 未指定外仓时为 `SCRIPT_DIR`；外仓时为 `$REPO/.harness` | Codex `-C`、当前目录、`artifacts/`、`project`（代码树）所在根。 |
...
`run_codex` / `run_codex_with_browser` 在失败时会按 **`CODEX_MAX_RETRIES`**、**`CODEX_RETRY_DELAY`** 重试（应对网络或 CLI 偶发错误），见下表。

### MCP 配置隔离

Codex CLI 不支持 `--mcp-config-file` 参数。Harness 通过**临时 `CODEX_HOME`** 实现 MCP 动态切换：
- **无浏览器步骤**（Planner / Generator / Contract）：临时目录下仅含基础 `config.toml`，不加载任何 MCP。
- **浏览器步骤**（Evaluator / Reviewer）：临时目录下 `config.toml` 注入 `[mcp_servers.playwright]`（`npx -y @playwright/mcp@latest --isolated`）。
- 用户的 `~/.codex/auth.json` 会被复制到临时目录，确保认证不失效。
- 临时目录在步骤结束后自动清理。
...
会在该仓库下创建 **`.harness/`**（含 `artifacts/` 与指向仓库根的 `project` 符号链接），Codex 的 `-C` 与状态文件均在此目录下。建议将 `.harness/` 或其中 `artifacts/` 加入该仓库的 `.gitignore`。
...
不传 `--project` / `HARNESS_PROJECT_ROOT` 时，工作区即本 `harness-codex` 目录：
...
| `CODEX_MAX_RETRIES` | `3` | `codex exec` 调用失败（含未产出约定产物）时的最大重试次数 |
| `CODEX_RETRY_DELAY` | `10` | 两次重试之间的等待秒数 |
...
- `codex` CLI (`@openai/codex`) 在 `PATH` 中，且已登录 (`codex login` 或设置了 `OPENAI_API_KEY`)
- `python3`（Python 3.11+，用于状态管理和 prompt 渲染）
- 评测阶段需 Playwright MCP：`npx -y @playwright/mcp@latest`（由 harness 自动通过临时 CODEX_HOME 注入）
