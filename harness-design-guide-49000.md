...
> **精简版说明（约 4.9 万「字」）**：可运行代码、完整英文 System Prompt、逐行示例等以同目录 [**`harness-design-guide.md`**](harness-design-guide.md) 为准。本文件为独立新文档。此处「字」指 **Unicode 码点**（如 Python `len(文本)`）；含中文的 UTF-8 文件在部分环境下 `wc -m` 可能接近字节数，与码点不等价。
